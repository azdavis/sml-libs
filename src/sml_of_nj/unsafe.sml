 ...
 * Unsafe by an ML program can cause corruption of the ML execution, which may lead to program
 * crashes or other insecurity.
 *)
signature UNSAFE = sig
   ...
   * this signature.) Throwing an exn to this continuation will behave as if the exception were
   * raised. There is nothing inherently unsafe about this (except the bogus use of 'a cont in the
   * signature instead of exn cont. It is even possible to grab the handler, then leave the dynamic
   * scope of the handler, then throw to it, with paradoxical but not unsafe effect.
   *)
  ...
  (*!
   * getVar () SML/NJ has a special global variable, kept in registers upon most target
   * architectures, used for various purposes by subsystems such as CML or the debugger. This
   * function fetches the contents of the "var".
   *)
  val getVar : unit -> 'a
  (*!
  ...
  (*!
   * topLevelCont The continuation to which control will be thrown when an interrupt signal (SIGINT,
   * on Unix) is received. Usually, the topLevelCont continuation resumes the interactive loop, but
   * in principle users could install other continuation values in this variable.
   *)
