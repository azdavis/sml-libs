signature SML_OF_NJ = sig
   ...
   * filename.arch-opsys (where arch is the machine architecture, such as "sparc" or "x86", and
   * opsys is the operating system, such as "solaris" or "win32"; and return false. When the heap
   * image is loaded into an SML runtime system, (with sml @SMLload=filename.arch-opsys, computation
   * will resume as if exportML had returned true.
   *)
   ...
   * system, (with sml @SMLload=filename.arch-opsys arg1 arg2 ..., computation will start with a
   * call to f(arg0,[arg1,arg2,...]) where arg0 is the name of the executable file (in this example,
   * the pathname expansion of sml). When f returns, the ML process terminates, with the process
  ...
  (*!
   * exnHistory exn Return the exception history of exn, showing where it was first raised and
   * through which exception-handlers it has been passed (or reraised). Each string in the list
   * describes the source-file and line number of the raising (or reraising) code; the most recent
   * handler (or reraise) is first, the earliest raising point is last. Code compiled with
