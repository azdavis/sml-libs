signature CONT = sig
  ...
  type 'a cont
  (*!
   * callcc f Apply f to the "current continuation". If f invokes this continuation with argument x,
   * it is as if (callcc f) had returned x as a result.
   *)
  ...
  (*!
   * isolate f x Discard all live data from the calling context (except what is reachable from f or
   * x), then call f(x), then exit. This may use much less memory then something like f(x) before
   * exit().
   *)
  ...
  type 'a control_cont
  (*!
   * capture f Apply f to the "current continuation". If f invokes this continuation with argument
   * x, it is as if (capture f) had returned x as a result, except that the exception-handler is not
