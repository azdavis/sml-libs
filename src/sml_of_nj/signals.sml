...
signature SIGNALS = sig
  ...
  (*!
   * maskSignals sigs Mask the specified set of signals: signals that are not IGNORED will be
   * delivered when unmasked. Calls to maskSignals nest on a per signal basis.
   *)
  val maskSignals : sigmask -> unit
  (*!
   * unmaskSignals sigs Unmask the specified signals. The unmasking of a signal that is not masked
   * has no effect.
   *)
  ...
  (*!
   * pause () Sleep until the next signal; if called when signals are masked, then signals will
   * still be masked when pause returns.
   *)
  val pause : unit -> unit
  (*!
  ...
  val sigTERM : signal
  (*!
   * sigGC Signalled by the runtime system (not by the operating system) immediately after each
   * garbage collection.
   *)
  val sigGC : signal
end
