(*!
 * The Unsafe.Poll structure provides an interface to automatic software polling. The pollEvent flag
 * is watched (with user-specified frequency), and when it becomes true, a user-specified function
 * is invoked. This is all done without hardware interrupts; it is useful where the operating system
 * does not provide an interface to hardware signals, or where the cost of hardware signal-handling
 * is too high.
 *
 ...
 * mk.<arch$GT;-<os$GT; make script in src/runtime/objs (see, for example, mk.x86-linux-poll) then,
 * do a "make clean" followed by a "make -f mk.<arch$GT;-<os$GT;"
 *
...
signature POLL = sig
   ...
   * invoked. pollEvent is accessible to the runtime system and may (of course) be set
   * asynchronously by other threads.
   *)
  val pollEvent : bool ref
  (*!
   * setHandler a If a=f , install f as the event handler. When pollEvent becomes true, the current
   * continuation k will be saved, pollEvent will be set to false, and f(k) will be called. No
   * polling will occur during the execution of f. When f returns some continuation k', then polling
   * will resume and k' will be invoked. If a=NONE , disable polling.
   *)
  ...
  (*!
   * setFreq a When a=(SOME i) , sets the polling frequency to i ; that is, approximately every
   * i*1000 instructions, the pollEvent variable will be checked. When a=NONE , disables polling.
   *)
